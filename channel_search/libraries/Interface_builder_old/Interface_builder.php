...
if(!class_exists('Interface_Builder'))
{
	...
	class Interface_Builder extends Interface_builder_core {
		...
		public function fieldsets($fieldsets = FALSE, $data = FALSE)
		{
			...
			{
				$wrapper = isset($fieldset->wrapper) ? $fieldset->wrapper : 'fieldset';
				
				$html[] = '<'.$wrapper.' id="'.$fieldset_id.'">';
	
				if(isset($fieldset->legend) && $fieldset->legend != NULL)
				{
				...
				if(isset($fieldset->description) && $fieldset->description != NULL)
				{
					$html[] = '<p>'.$fieldset->description.'</p>';
				}
	
				if(isset($fieldset->details) && $fieldset->details != NULL)
				{
					$html[] = '<div class="details">'.$fieldset->details.'</div>';
				}
	
				$html[] = $this->table($fieldset->fields, $data, isset($fieldset->attributes) ? $fieldset->attributes : array());
